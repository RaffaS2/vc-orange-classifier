	...
	/**
	 * @brief Valida se um blob corresponde a uma laranja.
	 *
	 * Verifica três condições: calibre mínimo de 53mm (CEE 379/71),
	 * fill ratio >= 0.70 (rejeita objetos com buracos) e bounding box
	 * completamente visível na imagem (rejeita objetos cortados).
	 *
	 * @param blob   Blob a validar.
	...
	{
		const int   margin    = 2;
		const float min_fill  = 0.70f;

		// 1. Tamanho mínimo — reutiliza calibre (retorna -1 se < 53mm)
		if (vc_orange_calibre(blob) == -1) return 0;

		// 2. Fill ratio — rejeita objetos com buracos excessivos (ex: maçã)
		float fill_ratio = (float)blob.area / (float)(blob.width * blob.height);
		if (fill_ratio < min_fill) return 0;

		// 3. Bounding box completamente visível — rejeita objetos cortados/a entrar
		if (blob.x                 <= margin)          return 0;
		if (blob.y                 <= margin)          return 0;
		if (blob.x + blob.width    >= width  - margin) return 0;
		if (blob.y + blob.height   >= height - margin) return 0;
